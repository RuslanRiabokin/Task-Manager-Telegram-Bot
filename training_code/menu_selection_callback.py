...
def get_keyboard_fab():
    ...
    for dish_name, dish_price in results:
        builder.button(
            ...
            callback_data=DishCallbackFactory(action="select", name=dish_name, price=dish_price)
        )

    builder.button(
        text="Подтвердить", callback_data=DishCallbackFactory(action="finish", name="", price=None)
    )
    # Выравниваем кнопки по 4 в ряд
    builder.adjust(1)
